...
さらに、すべてのスプリントに無料の**回帰ゲート**が加わります。テックリードがマイルストーン 1 でボットのベースラインを記録し、以降 QA はチケットを閉じる前にウェーブごとに `lab.js check` を実行します。回帰が見つかると原因となったチケットが再オープンされ、ボットが見つけたバグにはすべて、修正を証明するリプレイコマンドが付きます。
